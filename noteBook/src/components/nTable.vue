    ...
    <section class="event-all" :class="{'event-all-show':isShow}">
        <div class="table-box">
            <div class="edit-input"  :class="{'edit-input-show': is_edit}">
                <input type="text" v-model='info.content' ref='content'>
                <button @click='editData'>确定</button>
            </div>
            <div class="screen-box">
                <div class="div-select" :class="{'active':isActive}">
                    <div class="div-value" @click='isActive = !isActive'>{{getTypeName}}</div>
                    <div class="div-select-body">
                        <div class="div-option" @click='choseSelect(0)'>筛选类型</div>
                        <div class="div-option" @click='choseSelect(1)'>未完成</div>
                        <div class="div-option" @click='choseSelect(2)'>已完成</div>
                        <div class="div-option" @click='choseSelect(3)'>已取消</div>
                    </div>
                </div>
...
td button.del-btn,th button.del-btn{
    background: #F44336;
}
.edit-input{
    position: fixed;
...
.div-select.active .div-select-body{
    height:122px;
    border:1px solid #eee;
